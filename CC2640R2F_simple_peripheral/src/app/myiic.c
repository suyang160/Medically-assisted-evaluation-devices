...
static PIN_Handle sdaPinHandle;
static PIN_State sdaPinState;
static PIN_Handle sclPinHandle;
static PIN_State sclPinState;

PIN_Config SdaOutTable[] = {
    IOID_0 | PIN_GPIO_OUTPUT_EN | PIN_GPIO_LOW | PIN_PUSHPULL | PIN_DRVSTR_MAX,
    PIN_TERMINATE
};

PIN_Config SdaInTable[] = {
    IOID_0 | PIN_INPUT_EN  | PIN_PULLUP | PIN_HYSTERESIS,
    PIN_TERMINATE
}; 

PIN_Config SclOutTable[]={
...
void  SDA_IN()
{
    PIN_close(sdaPinHandle);
    sdaPinHandle = PIN_open(&sdaPinState, SdaInTable);	
}

void SDA_OUT()
{
    PIN_close(sdaPinHandle);
    sdaPinHandle = PIN_open(&sdaPinState, SdaOutTable);
}
void SDA_OUT1()
{
    sdaPinHandle = PIN_open(&sdaPinState, SdaOutTable);
...
void SCL_init()
{
    sclPinHandle = PIN_open(&sclPinState, SclOutTable); 
}

void IIC_SDA(uint_t temp)
{
    PIN_setOutputValue(sdaPinHandle, IOID_0, temp);
}

uint_t READ_SDA()
{
    return(PIN_getInputValue(IOID_0));
}

void IIC_SCL(uint_t temp)
{
    PIN_setOutputValue(sclPinHandle, IOID_1, temp);
}
